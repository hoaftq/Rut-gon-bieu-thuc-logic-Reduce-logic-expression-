...
CString CInputLogic::Str2Exp(const CString & rowbin){
	...
	if(bSumStandard){ //truong hop la tong chuan
		for(i=0; i<rowbin.GetLength(); i++){//duyet qua tat ca cac ky tu trng rowbin
			switch(rowbin[i]){
			case '1': //neu ky tu la 1 thi them ten bien
				if(!ret.IsEmpty()) ret+='.'; //neu khong la ky tu dau thi truoc khi them ten bien ta them dau va
				ret+=arrDimName[i]; break;
			case '0': //neu ky tu la 0 thi them dang: not(ten bien)
				if(!ret.IsEmpty()) ret+='.';//neu khong la ky tu dau thi truoc khi them not(ten bien) ta them dau va
		...
		for(i=0; i<rowbin.GetLength(); i++){//duyet qua tat ca cac ky tu trng rowbin
			...
			case '1': //neu ky tu la 1 thi them ten bien
				if(!ret.IsEmpty()){
					ret+='+';//neu khong la ky tu dau thi truoc khi them ten bien ta them dau va
					bNot1Term= true;
				}
				ret= ret+"not("+arrDimName[i]+")"; break;				
			case '0': //neu ky tu la 0 thi them dang: not(ten bien)
				if(!ret.IsEmpty()){
					ret+='+';//neu khong la ky tu dau thi truoc khi them not(ten bien) ta them dau va
					bNot1Term= true;
				}
				ret+=arrDimName[i];
			}
		}
		if(bNot1Term) ret= "("+ret+")"; //neu chi co 1 so hang trong tong cuc tieu thi khong can dat dau ngoac don
...
int CInputLogic::GetCount01(const CString & rowbin){
	...
	for(int i=0; i<rowbin.GetLength(); i++){
		if(rowbin.GetAt(i)=='0' || rowbin.GetAt(i)=='1') nRet++;
	}
	return nRet;
}
