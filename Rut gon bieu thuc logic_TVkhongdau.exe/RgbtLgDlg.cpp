...
BOOL CRgbtLgDlg::OnInitDialog(){
	...
	((CButton*)GetDlgItem(IDC_THEM))->SetWindowText("Them bien");
	m_nThem= 1;//them mac dinh ban dau la them bien
	
	m_cboDangBt.AddString("Tong chuan");
	m_cboDangBt.AddString("Tich chuan");
	m_cboDangBt.SetCurSel(0);
	m_nDangBt= 1;//bieu thuc dang tong chuan
	((CStatic*)GetDlgItem(IDC_TDDQUANTRONG))->SetWindowText("Cac dinh 1:");
	((CButton*)GetDlgItem(IDCR_THEM_2))->SetWindowText("Them dinh 1");

	m_bTang1= TRUE;
	((CButton*)GetDlgItem(IDCCHK_KQTOTNHAT))->SetCheck(TRUE);
	SetDlgItemText(IDC_RUTGON,"Rut gon ham logic : tong so dau vao o tang 1 la nho nhat!");
	return TRUE;
}
...
void CRgbtLgDlg::OnThem(){
	...
	if(m_nThem==1){//them bien
		...
		while(strThem.GetAt(0)=='0' && strThem.GetLength()> 1){
			strThem.Delete(0);
		}

		for(i=0; i<strThem.GetLength(); i++)
			if(strThem[i]<'0' || strThem[i]>'9') goto lblSetFocus;
...
void CRgbtLgDlg::OnSetFocusTenBien(){
	m_bntXoa.SetWindowText("Xoa ten bien");
	m_nXoa= 0;
}

void CRgbtLgDlg::OnSetFocusDQuanTrong(){
	if(m_nDangBt==1)
		m_bntXoa.SetWindowText("Xoa dinh 1");
	else 
		m_bntXoa.SetWindowText("Xoa dinh 0");
	m_nXoa= 1;
}

void CRgbtLgDlg::OnSetFocusDKQuanTrong(){
	m_bntXoa.SetWindowText("Xoa dinh khong xac dinh");
	m_nXoa= 2;
}
...
void CRgbtLgDlg::OnRutGon(){
	...
	if(nSoBien==0){MessageBox("Phai nhap bien cho ham !",NULL,MB_OK|MB_ICONINFORMATION); return;}
	if(m_lstDQuanTrong.GetCount()==0){
		if(m_nDangBt==1) MessageBox("Phai nhap dinh 1 !",NULL,MB_OK|MB_ICONINFORMATION);
		else MessageBox("Phai nhap dinh 0 !",NULL,MB_OK|MB_ICONINFORMATION);
		return;
	}
	int nNum;
	CString strMess;
	...
	for(i=0; i< m_lstDQuanTrong.GetCount(); i++){
		m_lstDQuanTrong.GetText(i,strItem);
		nNum= atoi((LPCSTR)strItem);
		if(nNum>= pow(2,nSoBien)){
			strMess.Format("Dinh xac dinh %d khong hop ly.",nNum);	
	...
	for(i=0; i< m_lstDKQuanTrong.GetCount(); i++){
		...
		nNum= atoi((LPCSTR)strItem);
		if(nNum>= pow(2,nSoBien)){
			strMess.Format("Dinh khong xac dinh %d khong hop ly.",nNum);	
			MessageBox(strMess,NULL,MB_OK|MB_ICONINFORMATION);
	...
	if(m_bTang1)
		strResult.Format(" (Ca(min) = %d, Toi thieu theo tieu chuan tong so dau vao o tang thu nhat la nho nhat)",tr.GetCaCb());
	else
		strResult.Format(" (Cb(min) = %d, Toi thieu theo tieu chuan tong so dau vao o ca hai tang la nho nhat)",tr.GetCaCb());
	((CEdit*)GetDlgItem(IDCE_KETQUA))->ReplaceSel(strResult); //viet tieu chuan
	((CEdit*)GetDlgItem(IDCE_KETQUA))->SendMessage(WM_CHAR,13,0); //xuong dong
	
	strResult.Format(" (Co %d ket qua)",tr.GetCountResult());
...
void CRgbtLgDlg::OnSelChangeDangBt(){
	...
	if(nCurSel==0){
		...
		((CStatic*)GetDlgItem(IDC_TDDQUANTRONG))->SetWindowText("Cac dinh 1:");
		((CButton*)GetDlgItem(IDCR_THEM_2))->SetWindowText("Them dinh 1");
		if(m_nThem==2) m_bntThem.SetWindowText("Them dinh 1");
		if(m_nXoa==1) m_bntXoa.SetWindowText("Xoa dinh 1");
	}else{
		m_nDangBt= 2;//dang tich chuan
		((CStatic*)GetDlgItem(IDC_TDDQUANTRONG))->SetWindowText("Cac dinh 0:");
		((CButton*)GetDlgItem(IDCR_THEM_2))->SetWindowText("Them dinh 0");
		if(m_nThem==2) m_bntThem.SetWindowText("Them dinh 0");
		if(m_nXoa==1) m_bntXoa.SetWindowText("Xoa dinh 0");
	}
}
...
void CRgbtLgDlg::OnClkOption(){
	...
	switch(m_nThem){
	case 1:
		((CButton*)GetDlgItem(IDC_THEM))->SetWindowText("Them bien"); break;
	case 2:
		if(m_nDangBt==1)
			((CButton*)GetDlgItem(IDC_THEM))->SetWindowText("Them dinh 1");
		else
			((CButton*)GetDlgItem(IDC_THEM))->SetWindowText("Them dinh 0");
		break;
	case 3:
		((CButton*)GetDlgItem(IDC_THEM))->SetWindowText("Them dinh khong xac dinh");
	}
...
void CRgbtLgDlg::OnKQTotNhat(){
	...
	if(m_bTang1)
		...
		SetDlgItemText(IDC_RUTGON,"Rut gon ham logic : tong so dau vao o ca hai tang la nho nhat!");
}

void CRgbtLgDlg::OnXoaKetQua(){
	SetDlgItemText(IDCE_KETQUA,"");
}
