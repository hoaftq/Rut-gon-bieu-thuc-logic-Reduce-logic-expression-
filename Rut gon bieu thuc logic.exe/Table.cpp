...
void SRow::Assign(CString str, bool used){
	...
	strBin= (PWCHAR)malloc(len+1);//cap phat vung nho cho thanh phan strBin
...
CString ConvertNum(UINT num,int CntChar){
	...
	while(CntChar>0){//lam CntChar lan		
		strdigit.Format(_T("%d"),num%2);//lay chu so nhi phan cuoi cung 		
...
CString CombineStr(CString rowbin1, CString rowbin2){
	...
	if((len= rowbin1.GetLength())!= rowbin2.GetLength()) return _T("");
	...
	for(i=0; i<len; i++){
		if(rowbin1.GetAt(i)!= rowbin2.GetAt(i)){
			if((rowbin1.GetAt(i)== _T('_'))||(rowbin2.GetAt(i)== _T('_'))) return _T("");
			break;
		}
	}
	...
	if(i< len){
		for(int j=i+1; j< len; j++)
			if(rowbin1.GetAt(j)!= rowbin2.GetAt(j)) return _T("");
		result= rowbin1;
		result.SetAt(i,_T('_'));
		return result;
	}else return _T("");
		...
		for(int j= 0; j< por2.iCntRow; j++){//ung voi moi hang cua por1 duyet qua tat ca cac hang trong por2
			...
			if(strCom!=_T("")){//neu ket hop duoc
				...
				for(int k= 0; k< iCntRow; k++)
					if(wcscmp(strCom,pRow[k].strBin)==0) goto notadd; //(LPCSTR)
